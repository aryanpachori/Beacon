...
import { getInstallUrl, buildOAuthState, parseOAuthState, resolveFrontendUrl, getOAuthAuthorizeUrl, exchangeOAuthCode } from '../lib/github'
...
githubRouter.get('/oauth/url', authMiddleware, (req: AuthRequest, res, next) => {
  try {
    const state = buildOAuthState(req.user!.userId, readOrigin(req))
...
githubRouter.get('/oauth/callback', async (req, res, next) => {
  try {
    ...
    if (!userRes.ok) {
      return res.redirect(`${frontendUrl}/onboarding?step=1&error=github_oauth`)
    }
    ...
    const installation = await linkInstallationForGitHubUser(ghUser.login, userId)
    if (!installation) {
      return res.redirect(`${frontendUrl}/onboarding?step=1&error=no_installation`)
    }

    try {
      await syncInstallationRepos(installation.id, Number(installation.installationId))
    } catch (err) {
...
githubRouter.get('/onboarding/state', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    const user = await prisma.user.findUnique({
      where: { id: req.user!.userId },
      select: { onboardingStep: true, plan: true, onboardingCompletedAt: true },
    })
    ...
    res.json({
      ...
      onboardingComplete: !!user.onboardingCompletedAt && total > 0,
      scanStatus,
      total,
...
githubRouter.get('/callback', async (req, res, next) => {
  try {
    ...
    const accountLogin = await resolveInstallationAccountLogin(installationIdNum)
...
githubRouter.get('/onboarding/repos', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    const user = await prisma.user.findUnique({
      where: { id: req.user!.userId },
      select: { plan: true },
...
githubRouter.post('/start-scan', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
...
githubRouter.get('/onboarding/status', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
...
githubRouter.get('/onboarding/stream', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    ...
    const unsubscribe = subscribeScanProgress(userId, (payload) => {
      ...
      if (payload.status === ScanStatus.complete || payload.status === ScanStatus.failed) {
        res.end()
      }
    })
