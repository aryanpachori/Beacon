      ...
      {SIGNAL_KEYS.map(key => {
        ...
        return (
          <div key={key} className="group/sig relative flex flex-col items-center">
            ...
            <div className="pointer-events-none absolute bottom-full mb-1.5 hidden rounded-lg border border-[#e4e8ee] bg-white px-2 py-1 shadow-lg group-hover/sig:block whitespace-nowrap z-10">
              <p className="text-[10px] font-semibold text-[#1e2a3c]">{SIGNAL_LABELS[key]}</p>
              <p className="text-[10px] text-[#9fa0b5]">{Math.round(val)}/100</p>
            </div>
          </div>
        )
      })}
...
function TrendIndicator({ history }: { history: number[] }) {
  if (history.length < 2) return <Minus className="h-3.5 w-3.5 text-[#c6d1d7]" />
  ...
  return <Minus className="h-3.5 w-3.5 text-[#c6d1d7]" />
}
...
function MaintainerAvatar({ pkg }: { pkg: Package }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="max-w-[80px] truncate text-[11px] text-[#9fa0b5]">{label}</span>
    </div>
  )
}
...
export function PackageListRow({ pkg, index, searchQuery = '' }: PackageListRowProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative flex items-center gap-4 border-b border-[#f0f2f5] bg-white px-5 py-3.5 last:border-0 transition-colors hover:bg-[#fafbfc]"
    >
      ...
      <div className="flex w-[220px] shrink-0 items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <code className="text-[10px] text-[#9fa0b5] font-mono">{pkg.version}</code>
            {pkg.isDeprecated && (
      ...
      <div className="hidden w-[120px] shrink-0 sm:block">
        <p className="truncate text-[11px] font-medium text-[#9fa0b5]">{pkg.repoName.split('/')[1] ?? pkg.repoName}</p>
      </div>
      ...
      <div className="hidden shrink-0 lg:flex items-center">
        {pkg.scoringPending ? (
          <span className="text-[11px] text-[#c6d1d7]">Scoring…</span>
        ) : (
      ...
      <div className="hidden shrink-0 md:block">
        {pkg.scoringPending ? (
          <span className="rounded-full bg-[#f0f2f5] px-2 py-1 text-[10px] font-semibold text-[#9fa0b5]">Pending</span>
        ) : (
      ...
      <div className="flex shrink-0 items-center gap-2">
        <span className="hidden text-[11px] text-[#9fa0b5] sm:block">{relTime}</span>
        <Link
          href={`/packages/${pkg.id}`}
          className={cn(
            'flex h-7 w-7 items-center justify-center rounded-lg border border-[#e4e8ee] text-[#9fa0b5]',
            'opacity-0 transition-all group-hover:opacity-100 hover:border-[#2f7eda]/30 hover:bg-[#eaf2fd] hover:text-[#2f7eda]'
          )}
        >
