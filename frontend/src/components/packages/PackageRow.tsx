...
export function PackageRow({ pkg, index }: PackageRowProps) {
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-3 text-sm text-dash-muted font-mono">{pkg.version}</td>
      <td className="px-4 py-3 text-sm text-dash-muted">{pkg.repoName.split('/')[1]}</td>
      <td className="px-4 py-3">
      ...
      <td className="px-4 py-3 text-xs text-dash-muted">{timeAgo(pkg.lastUpdated)}</td>
    </motion.tr>
  )
}
