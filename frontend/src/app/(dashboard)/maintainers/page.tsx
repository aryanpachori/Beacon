      ...
      {!loading && maintainers.length > 0 && (
        <div className="mb-5 grid grid-cols-3 gap-3">
          {[
            ...
            <div key={label} className="rounded-2xl border border-[#e4e8ee] bg-white p-4">
              <p className="text-[22px] font-bold text-[#1e2a3c]">{value}</p>
              <p className="mt-0.5 text-[11px] font-semibold" style={{ color }}>{label}</p>
              <div className="mt-2 h-1 overflow-hidden rounded-full bg-[#f0f2f5]">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-[#e4e8ee] bg-white py-20 text-center">
          <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#f5f7fa]">
            <Users className="h-7 w-7 text-[#c6d1d7]" />
          </div>
          <p className="text-[15px] font-semibold text-[#1e2a3c]">No maintainer data yet</p>
          ...
          {filtered.map((m, i) => {
            ...
            return (
              <motion.div
                ...
                className="flex flex-col gap-4 rounded-2xl border border-[#e4e8ee] bg-white p-5 transition-shadow hover:shadow-md"
              >
                {/* Header */}
                <div className="flex items-start gap-3">
                  {/* Avatar */}
                  {m.avatarUrl ? (
                    <img src={m.avatarUrl} alt={m.displayName} className="h-10 w-10 rounded-full border border-[#e4e8ee]" />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="truncate text-[14px] font-bold text-[#1e2a3c]">{m.displayName}</p>
                    <a
                      href={`https://github.com/${m.login}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1 text-[11px] text-[#9fa0b5] hover:text-[#2f7eda] transition-colors"
                    >
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="rounded-xl bg-[#f5f7fa] px-2 py-2 text-center">
                    <div className="flex items-center justify-center gap-0.5 text-[11px] font-bold text-[#1e2a3c]">
                      <TrendingDown className="h-3 w-3 text-[#9fa0b5]" />
                      {m.daysInactive}d
                    </div>
                    <p className="text-[9px] text-[#9fa0b5] mt-0.5">inactive</p>
                  </div>
                  <div className="rounded-xl bg-[#f5f7fa] px-2 py-2 text-center">
                    <div className="flex items-center justify-center gap-0.5 text-[11px] font-bold text-[#1e2a3c]">
                      <BookOpen className="h-3 w-3 text-[#9fa0b5]" />
                      {m.publicReposCount}
                    </div>
                    <p className="text-[9px] text-[#9fa0b5] mt-0.5">repos</p>
                  </div>
                  <div className="rounded-xl bg-[#f5f7fa] px-2 py-2 text-center">
                    <div className="flex items-center justify-center gap-0.5 text-[11px] font-bold text-[#1e2a3c]">
                      <Star className="h-3 w-3 text-[#9fa0b5]" />
                      {m.followersCount}
                    </div>
                    <p className="text-[9px] text-[#9fa0b5] mt-0.5">followers</p>
                  </div>
                </div>

                {/* Package list */}
                <div>
                  <p className="mb-1.5 text-[10px] font-bold uppercase tracking-[0.1em] text-[#c6d1d7]">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {m.packages.slice(0, 4).map(pkg => (
                      <Link
                        ...
                        className="rounded-lg border border-[#e4e8ee] bg-[#f5f7fa] px-2 py-1 text-[11px] font-semibold text-[#555663] hover:border-[#2f7eda]/30 hover:bg-[#eaf2fd] hover:text-[#2f7eda] transition-all"
                    ...
                    {m.packages.length > 4 && (
                      <span className="rounded-lg bg-[#f0f2f5] px-2 py-1 text-[11px] text-[#9fa0b5]">
