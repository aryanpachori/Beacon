...
export type ApiDashboard = {
  healthCounts: { healthy: number; watch: number; atRisk: number; critical: number }
...
function extractSignalsPayload(raw: unknown): {
  ...
  if (Array.isArray(raw)) {
    return { normalized: dbSignalsToNormalized(raw), facts: null, maintainers: [] }
  }
...
function dbSignalsToNormalized(signals: ApiDbSignal[]): NormalizedSignals | null {
...
export function normalizedToPackageSignals(signals: NormalizedSignals): Package['signals'] {
  return {
...
export function parseSignals(raw: ApiPackageDetail['signals']): Package['signals'] {
